...
export class MessageInputComponent implements OnInit {
  ...
  // users to be used for autocomplete (ARRAY OF OBJECTS)
  users: User[] = [
    { id: '1', username: 'Andy' },
    { id: '2', username: 'Bobby' },
    { id: '3', username: 'Gary' },
    ...
    { id: '6', username: 'Holt' },
    { id: '7', username: 'Rosa' },
    { id: '8', username: 'Santiago' }
  ]

  // Array for holding the filtered user objects
  // the usernames of these objects will be shown in autocomplete box
  filtered: User[] = [];
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if (this.suggestionsBoxVisible) {
      ...
      if (event.keyCode === 38) {
        ...
        let currentSelectedIndex = this.filtered.indexOf(this.selected);
        if (currentSelectedIndex === 0) {
          // Wrap around
          // if its the first element, then set it to the last
          this.selected = this.filtered[this.filtered.length - 1];
        } else {
          // otherwise decrease the index (so the selected goes nearer the top)
          this.selected = this.filtered[currentSelectedIndex - 1];
        }
        ...
        let currentSelectedIndex = this.filtered.indexOf(this.selected);
        if (currentSelectedIndex === this.filtered.length - 1) {
          // Wrap around
  ...
  autoComplete() {
    if (this.selected) {
      ...
      if (this.selected.username.length >= current.value.slice(1).length) {
        ...
        let newInputValue
          ...
          + inputValue.slice(current.endIndex + 1);

        this.inputEl.value = newInputValue;

        // generate the new cursor position
        let newCursorPos = current.startIndex + this.selected.username.length + 2;
  ...
  handleSuggestions() {
    ...
    if (wordBeingTyped.startsWith('@') && !this.getSelectedText().includes(' ')) {
      ...
      // Filter and show users in the suggestions box
      let cache = this.filtered;
      this.filterUsers(wordBeingTyped);


      if (this.filtered.length === 0) {
        this.hideSuggestionsBox();
      } else if (cache.length != this.filtered.length) {
  ...
  /**
   * Updates the filtered array based on a query
